...
**小结**：本节我们从 Netflix Prize 讲起，介绍了矩阵分解的核心思想，并用 Numpy 实现了 FunkSVD 和 BiasSVD。矩阵分解通过学习用户和物品的低维隐向量，优雅地解决了协同过滤的稀疏性问题，并为后来的 Embedding 技术奠定了基础。下一节我们将转向线性模型，看看如何用 Logistic Regression 做 CTR 预估。
