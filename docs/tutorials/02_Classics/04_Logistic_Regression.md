...
|------|---------|------|------|---------|---------|
| **UserCF** | 找相似用户，推荐他们喜欢的物品 | 简单直观、可解释性强 | 稀疏性问题、用户冷启动 | HR@10: 42.68% | 新闻推荐、社交推荐 |
| **ItemCF** | 找相似物品，推荐给喜欢类似物品的用户 | 物品关系稳定、易于缓存 | 物品冷启动、多样性不足 | HR@10: 14.43% | 电商推荐、内容推荐 |
| **矩阵分解** | 学习用户和物品的隐向量表示 | 缓解稀疏性、引入隐语义 | 只适合显式评分场景 | HR@10: 5.20% | 评分预测（如电影评分） |
| **逻辑回归** | 线性分类器，输出点击概率 | 快速、可解释、易于在线学习 | 需要特征工程、无法捕捉交叉 | AUC: 0.53 | CTR预估基线、广告排序 |

**关键洞察**：
- **UserCF在新闻场景表现最好**（HR@10=42.68%），因为用户兴趣相对稳定，而新闻时效性强
- **矩阵分解效果最差**（HR@10=5.20%），因为MIND是隐式反馈数据，而FunkSVD是为显式评分设计的
- **逻辑回归的AUC只有0.53**，说明简单的统计特征不足以捕捉复杂的点击模式，需要更丰富的特征或更强大的模型
