...
其中 $u$ 是用户，$q$ 是查询语句，$t$ 是候选物料。
在搜索中，**Query ($q$) 享受绝对的 VIP 待遇**。最重要的特征往往是 $q$ 与 $t$ 的交叉特征（如文本匹配度）。如果用户输入"苹果"，系统必须返回水果或手机，绝不能因为该用户喜欢吃香蕉就给他展示香蕉。

**推荐模型**可以抽象为：
$$ F_{reco}(u, t) \rightarrow Score $$
这里没有显式的 $q$，系统完全依赖用户画像 $u$（包括历史行为序列）与物料 $t$ 的匹配。
...
假设用户搜索"二战 德国 坦克"，用户的意图极其明确。搜索结果 $R_{search}$ 必须满足：
$$ R_{search} = \text{Tag(二战)} \cap \text{Tag(德国)} \cap \text{Tag(坦克)} $$
结果必须同时包含这三个要素。如果系统返回了"二战苏联飞机"，会被认为是严重的 **Bug**（相关性错误）。搜索对准确性的要求是刚性的。

**推荐遵循"并集"逻辑（Union）**：
假设同一个用户的画像中包含"二战"、"德国"、"坦克"这三个兴趣标签。推荐系统为了挖掘兴趣、增加多样性，其结果 $R_{reco}$ 往往是：
$$ R_{reco} \approx \text{TopN}( \text{Tag(二战)} \cup \text{Tag(德国)} \cup \text{Tag(坦克)} ) $$
系统不仅可以推德国坦克，还可以推"二战历史"（宽泛兴趣），甚至基于关联规则推"二战苏联飞机"（相关兴趣扩展）。这种扩展在推荐中不仅不是 Bug，反而是**惊喜**（Serendipity）。

## 商业层：推搜 vs. 广告 —— 利益博弈与精度要求
...
**推荐与搜索只需要"相对准确"**：
假设有两个物品 A 和 B，用户对 A 的真实点击率是 10%，对 B 是 5%。
