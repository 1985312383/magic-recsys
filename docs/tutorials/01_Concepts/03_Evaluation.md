...
<!-- TODO(图): docs/public/img/ch01/evaluation_overview.png
建议内容：评测体系全景图 - 四大板块：离线评测(Precision/Recall/NDCG)、在线指标(CTR/CVR/留存)、魔法指标(多样性/覆盖率/惊喜度)、A/B Test -->
...
离线评测的基本做法是：把历史日志切成训练集和测试集，让模型在训练集学习，在测试集上打分。这里有个非常重要的事实：离线评测里我们并不知道"用户真正喜欢的全集"。所谓的"正确答案"，通常用测试集里的正反馈来近似（例如：用户在某个时间窗里点击/购买过的物品）。所以离线评测衡量的是"能否复现历史行为"，而不是"上线后一定更好"。
...
想象你是奶茶店店员，给客人推荐 10 款饮品：芝士奶盖、多肉葡萄、柠檬茶、珍珠奶茶、草莓啵啵、杨枝甘露、红豆奶茶、乌龙茶、冰淇淋红茶、焦糖玛奇朵。客人最后点了其中 3 款：芝士奶盖、多肉葡萄、杨枝甘露。**Precision@K（准确率）** 就是 3/10 = 30%，衡量"你推荐的东西里，有多少是靠谱的"。公式表达为：
...
**Retention（留存率）** 是衡量产品生死的王牌指标。如果你为了短期 CTR 拼命推低俗内容，用户明天卸载了，那留存率就崩了。公式为 $\text{次日留存率}=\frac{\text{第 }t+1\text{ 天仍活跃的人数}}{\text{第 }t\text{ 天的活跃人数}}$。比如 1 月 1 日有 1000 个新用户，1 月 2 日其中 600 人又打开了 App，次日留存率 = 60%。移动 App 的行业参考值：D1 留存 > 40% 是优秀，< 20% 是危险。为什么留存比 CTR/CVR 更重要？因为拉新成本很高，如果用户用一次就跑了，公司会亏本。只有留存高，用户才会持续产生价值。

除了核心指标，还要盯着一些"护栏指标"防止系统跑偏：时延（推荐响应速度，通常要求 < 200ms）、负反馈率（用户点"不感兴趣"的比例）、投诉率（推荐违规内容的比例）等。

<!-- TODO(图): docs/public/img/ch01/online_metrics_funnel.png
建议内容：在线指标漏斗图 - 曝光 → 点击(CTR) → 转化(CVR) → 留存(Retention)，逐层递减的漏斗形态 -->
...
<!-- TODO(图): docs/public/img/ch01/magic_metrics_tradeoff.png
建议内容：CTR vs 多样性/惊喜度的权衡曲线图 - X轴：多样性/惊喜度，Y轴：CTR，展示负相关关系及最优平衡点 -->
...
再看一个推荐系统实战案例。假设你是抖音的算法工程师，开发了一个新的推荐模型（加入了多样性优化）。A 组 50% 用户使用旧模型（纯 CTR 优化），B 组 50% 用户使用新模型，运行 2 周。结果：CTR 方面，A 组 22.5%，B 组 22.1%，下降 0.4%（不显著）；人均停留时长方面，A 组 35 分钟，B 组 42 分钟，提升 +20%（显著）；次日留存率方面，A 组 58%，B 组 64%，提升 +6%（显著）。分析：CTR 略微下降，但停留时长和留存率大幅提升。综合判断：新模型更好，上线！
...
**总结**：评测体系是推荐系统的"体检报告"。离线评测快速筛选，排除明显不合格的方案；在线指标观察真实商业价值（CTR/CVR/留存）；魔法指标守护长期用户体验（多样性/覆盖率/惊喜度）；A/B Test 是最终裁判，用真实数据决定上线与否。没有 A/B Test，就没有数据驱动的决策。这是推荐工程师必须掌握的核心武器。

<!-- TODO(图): docs/public/img/ch01/ab_test_flow.png
建议内容：A/B Test流程图 - 用户随机分流 → A组(对照组/旧策略) vs B组(实验组/新策略) → 数据收集 → 指标对比 → 决策上线/回滚 -->
